class WorkExperience < ApplicationRecord
  ...
  validates :employment_type, presence: true, inclusion: { in: EMPLOYMENT_TYPE,
             message: 'not a valid employment type' }
  validates :location_type, presence: true, inclusion: { in: LOCATION_TYPE,
             message: 'not a valid location type' }
  ...
  def company_with_employement_type
  ...
  def job_duration
    ...
    if currently_working_here
      ...
      "#{end_date.strftime("%b %Y")} - #{start_date.strftime("%b %Y")}"
    end
  end

  def fetch_months(last_date)
    ((last_date.year - start_date.year) * 12 + last_date.month - start_date.month 
